...
EPHEMERAL_NAME="myproject-rm-${USER_NAME}"
DEFAULT_PERSISTENT_NAME="myproject-default-${USER_NAME}"

# --gpus is fixed at `docker run`; older persistent containers must be recreated.
GPU_FLAGS=()
...
# Helper: List all persistent containers of form myproject-<base>-<username>
list_persistent_all() {
  docker ps -a --format '{{.Names}}\t{{.Image}}\t{{.Status}}' |
    awk -v user="${USER_NAME}" -F '\t' '$1 ~ ("^myproject-.*-"user"$") { print $0 }'
}

# Helper: Ensure name is normalized to: myproject-<base>-<username>
normalize_name() {
  local raw="$1"
  local prefix="myproject-"
